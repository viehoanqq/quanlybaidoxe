...
namespace CarParkingManagementSystem
{
	public partial class formParkingSpaceDetail : Form
	{
		...
		{
			this.comboxtype.SelectedItem = 0;
			this.txtID.Clear();
		...
		public void LoadData(string id)
		{
			...
			this.comboxtype.Text = dt.Rows[0]["type"].ToString();
			this.checkdattruoc.Checked = bool.Parse(dt.Rows[0]["isBooked"].ToString());
		}
		...
		private void btnSave_Click(object sender, EventArgs e)
		{
			...
			if (flag == 0)
			{
				try
				{
					if (manager.ThemNoiDoXe(txtID.Text, txtIDKhachhang.Text, txtIDNhanvien.Text, txtTrangthai.Text, txtGiatien.Text, comboxtype.Text))
					{
			...
			else if (flag == 1)
			{
				try
				{
					if (manager.SuaNoiDoXe(txtID.Text, txtIDKhachhang.Text, txtIDNhanvien.Text, txtTrangthai.Text, txtGiatien.Text, comboxtype.Text , checkdattruoc.Checked))
					{
