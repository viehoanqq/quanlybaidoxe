...
namespace CarParkingManagementSystem.BSLayer
{
	public class Manager
	{
		...
		public bool ThemNoiDoXe(string IDDoXe, string IDNguoiDung, string IDNhanVien, string trangthai, string sotien, string type)
		{
			string sql = "INSERT INTO NoiDoXe (IDDoXe, IDNguoiDung, IDNhanVien, trangthai, sotien, type) " +
						 "VALUES (@IDDoXe, @IDNguoiDung, @IDNhanVien, @trangthai, @sotien, @type)";
		...
		public bool SuaNoiDoXe(string IDDoXe, string IDNguoiDung, string IDNhanVien, string trangthai, string sotien, string type , bool check)
		{
			string sql = "UPDATE NoiDoXe SET " +
			...
			cmd.Parameters.AddWithValue("@sotien", sotien);
			cmd.Parameters.AddWithValue("@type", type);
			cmd.Parameters.AddWithValue("@isBooked", check);
		...
		{
			DataTable dt = new DataTable();
			SqlCommand cmd = new SqlCommand("select * from Discount", db.GetConnection());
			SqlDataAdapter adapter = new SqlDataAdapter(cmd);
			adapter.Fill(dt);
			return dt;
		}

		public bool AddDiscount(string tenMa, decimal phanTramGiam, DateTime ngayBatDau, DateTime ngayKetThuc)
		{
			string query = @"
				INSERT INTO Discount (tenMa, phanTramGiam, ngayBatDau, ngayKetThuc)
				VALUES (@tenMa, @phanTramGiam, @ngayBatDau, @ngayKetThuc)";
		...
		public DataTable FinDiscountWithID(string param)
		{
			DataTable dt = new DataTable();
			string query = @"
					SELECT * FROM Discount
					WHERE 
						CAST(ID AS NVARCHAR) LIKE @param OR
						tenMa LIKE @param OR
						CONVERT(NVARCHAR, phanTramGiam) LIKE @param OR
						CONVERT(NVARCHAR, ngayBatDau, 120) LIKE @param OR
						CONVERT(NVARCHAR, ngayKetThuc, 120) LIKE @param;
				";
		...
		public bool UpdateDiscount(int id, string tenMa, decimal phanTramGiam, DateTime ngayBatDau, DateTime ngayKetThuc)
		{
			string query = @"
				UPDATE Discount
				SET tenMa = @tenMa,
			...
			return result > 0; // true nếu sửa thành công
		}
		...
		public DataTable GetDisCountWithID(string id)
		{
			DataTable dt = new DataTable();
